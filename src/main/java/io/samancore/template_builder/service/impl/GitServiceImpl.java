...
public class GitServiceImpl implements GitService {
    ...
    public Node getProduct(String product, String branch, String token) {
        var file = PRODUCTS.concat(product).concat(SLASH).concat(INFO_FILE);
        return client.getFile(file, branch, token);
    }

    @Override
    public List<Node> listTemplates(String product, String branch, String token) {
        var path = PRODUCTS.concat(product).concat(SLASH).concat(TEMPLATES);
    ...
    public Node getTemplateJson(String product, String template, String branch, String token) {
        var file = PRODUCTS.concat(product).concat(SLASH).concat(TEMPLATES).concat(SLASH).concat(template).concat(SLASH).concat(FORM_FILE);
        return client.getFile(file, branch, token);
    }
    ...
    public ConditionsProperty getConditionsProperty(String product, String template, String property, String branch, String token) {
        var path = PRODUCTS.concat(product).concat(SLASH).concat(TEMPLATES).concat(SLASH).concat(template).concat(SLASH).concat(CONDITIONS);
    ...
    public Node getConditionProperty(String product, String template, String property, ConditionType type, String branch, String token) {
        ...
        var file = PRODUCTS.concat(product).concat(SLASH).concat(TEMPLATES).concat(SLASH).concat(template).concat(SLASH).concat(CONDITIONS).concat(SLASH).concat(conditionName);
        return client.getFile(file, branch, token);
    }
